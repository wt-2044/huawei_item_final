...
require.config({
    ...
    paths: { //模块路径,路径引入后缀名必须忽略，配置里面已经存在后缀名（扩展名）。
        ...
        'jquery_pagination': 'http://192.168.11.8/js.two/huawei_item/src/script/jquery.pagination'
    },
    shim: { //不支持AMD模块，支持AMD
        'jq_cookie': {
            deps: ['jquery'], //模块的依赖
...
// 加载模块
require(['jquery', 'jq_cookie', 'jq_lazyload', 'jquery_pagination'], function() {
